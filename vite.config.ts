...
export default defineConfig(({ mode }) => {
  ...
  // The ML host serves its routes at ROOT (`/chat/stream`), so the same-origin
  // `/ml-api` prefix is stripped on the way through (unlike the main `/api/v1`
  // proxy, which keeps its prefix). Only wired when the ML URL is a path.
  if (mlProxyTarget && mlBase.startsWith('/')) {
    proxy[mlBase] = {
      target: mlProxyTarget,
      changeOrigin: true,
      secure: true,
      rewrite: (path: string) => path.replace(new RegExp(`^${mlBase}`), ''),
      headers: { 'ngrok-skip-browser-warning': 'true' },
    }
  }

/**
 * Shared account-context building blocks.
 *
 * Every export here renders CONTENT ONLY — no surrounding box, and (mostly) no
 * heading — so the two surfaces that present account context can compose them
 * with their own chrome without the markup drifting apart:
 *   • AccountPanel — the persistent lg+ sidebar: wraps each in a CARD_CLASS box
 *     under a SectionHeading.
 *   • AccountRail  — the below-lg icon rail: drops one card at a time into a
 *     flyout / bottom sheet under a PanelHeader.
 *
...
export const HEADING_CLASS = 'text-brand-navy dark:text-slate-100'
export const LABEL_CLASS = 'text-slate-500 dark:text-slate-400'

/** The icon + label heading a card carries in the sidebar. */
export function SectionHeading({ icon: Icon, children }: { icon: typeof Bell; children: ReactNode }) {
  return (
...
/** Customer identity line — name, customer no. / since, and status badge. */
...
/** Account chips for multi-account customers; nothing for single-account ones. */
export function AccountSwitcher() {
  const dispatch = useAppDispatch()
  const { customer, account } = useAccountContext()
  if (!customer || !account || customer.accounts.length <= 1) return null
  return (
...
/** Contact + service rows: email, phone, service address, meter. */
...
/** Balance amount, due date, and the autopay chip. */
    ...
    <div className="flex items-end justify-between">
      <div>
        ...
        <p className={cn('flex items-center gap-1 text-xs', LABEL_CLASS)}>
          ...
          {account.balance > 0 ? t('account.due', { date: account.dueDate }) : t('account.nothingDue')}
        </p>
      </div>
      <span
        className={cn(
...
/**
 * Usage history: range toggle, bar chart, stats grid, and the YoY indicator.
 *
 * `label` sits on the toggle row (the sidebar passes "Usage (unit)"); omit it —
 * as the rail does — and the toggle stays right-aligned on its own, the panel
 * header already naming the card.
 */
export function UsageContent({ label }: { label?: ReactNode }) {
  const { t } = useTranslation('copilot')
  const { account } = useAccountContext()
  const [range, setRange] = useState<UsageRange>('1Y')

  // Reset the range whenever the account changes so the behaviour is symmetric.
  useEffect(() => setRange('1Y'), [account?.id])
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-2">
        {label ? (
          <p className={cn('text-sm font-medium', HEADING_CLASS)}>{label}</p>
        ) : (
          <span aria-hidden />
        )}
...
/** The account's notices, most recent first. */
